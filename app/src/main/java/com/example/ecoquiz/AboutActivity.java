...
public class AboutActivity extends AppCompatActivity implements AboutFragment.BroListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportFragmentManager().beginTransaction().replace(R.id.aboutFragmentContainer, aboutFrag).addToBackStack(null).commit();
    }

    @Override
    public void onLongClick() {
        AboutSecret secretFrag = new AboutSecret();
    getSupportFragmentManager().beginTransaction().replace(R.id.aboutFragmentContainer, secretFrag).commit();
    }
}
